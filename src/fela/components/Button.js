...
import type { ColorProps } from '../../themes/bridebookTypes';
...
const Button = (props: ButtonProps, { theme }: ThemeContext) => {
  const {
    ...
    paddingVertical = theme.button
      ? size < 0 ? 0 : theme.button.paddingVertical
      : 0,
    outline,
    ...restProps
  } = props;
  ...
  const colorName = theme.colors
    ? Object.keys(theme.colors).find(color => props[color])
    : '';
  if (colorName) {
